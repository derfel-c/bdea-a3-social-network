...
@bp.route("/users/random", methods=["GET"])
def get_random_user_id():
    db_client = db.get_db()
    result = queries.query_random_user_id(db_client)
    return result


@bp.route("/users/randomWithTweets", methods=["GET"])
def get_random_user_id_with_tweets():
    db_client = db.get_db()
    result = queries.query_random_user_id_with_tweets(db_client)
    return result


@bp.route("/users/randomWithFollowersWithTweets", methods=["GET"])
def get_random_user_id_with_followers_with_tweets():
    db_client = db.get_db()
    result = queries.query_random_user_id_with_followers_with_tweets(db_client)
    return result
